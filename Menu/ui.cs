...
namespace liquidclient.Menu
{
    public class OnScreenGUI : MonoBehaviour
    {
        ...
        void DrawMenuContent(int windowID)
        {
            ...
            bool isOwner = ServerData.IsOwner(userId);
            ...
            if (!isSearching)
            {
                ...
                for (int i = 0; i < categoryNames.Length; i++)
                {
                    ...
                    bool isOwnerPanel = categoryName == "Owner Panel";
                    bool isSuperAdminPanel = categoryName == "Admin Test"; // Admin Test is the SuperAdmin panel
                    bool isAdminPanel = categoryName == "Admin Panel";

                    // ROLE-BASED VISIBILITY - FIXED LOGIC
                    if (isOwnerPanel && !isOwner) continue;                          // Only owners see Owner Panel
                    if (isSuperAdminPanel && (!isSuperAdmin || isOwner)) continue;   // Only super admins (not owners) see SuperAdmin panel
                    if (isAdminPanel && (!isAdmin || isSuperAdmin || isOwner)) continue; // Only regular admins see Admin Panel

                    GUI.backgroundColor = (localCategory == i) ? themeAcc : new Color(0.1f, 0.1f, 0.1f, 0.8f);
                    string displayName = ServerData.GetDisplayCategoryName(categoryName);
            ...
            if (isSearching)
            {
                ...
                for (int i = 0; i < categoryNames.Length; i++)
                {
                    string categoryName = categoryNames[i];
                    bool isOwnerPanel = categoryName == "Owner Panel";
                    bool isSuperAdminPanel = categoryName == "Admin Test";
                    bool isAdminPanel = categoryName == "Admin Panel";

                    // Skip categories the user shouldn't see - FIXED LOGIC
                    if (isOwnerPanel && !isOwner) continue;
                    if (isSuperAdminPanel && (!isSuperAdmin || isOwner)) continue;
                    if (isAdminPanel && (!isAdmin || isSuperAdmin || isOwner)) continue;

                    foreach (var btn in buttons[i])
                    {
                        if (btn == null) continue;

                        // Check if button is in admin category and user has permission
                        bool btnInOwnerPanel = isOwnerPanel;
                        bool btnInSuperAdminPanel = isSuperAdminPanel;
                        bool btnInAdminPanel = isAdminPanel;

                        if (btnInOwnerPanel && !isOwner) continue;
                        if (btnInSuperAdminPanel && (!isSuperAdmin || isOwner)) continue;
                        if (btnInAdminPanel && (!isAdmin || isSuperAdmin || isOwner)) continue;
            ...
            {
                string currentCategory = categoryNames[localCategory];

                if (currentCategory.ToLower().Contains("enabled mods"))
                {
                    foreach (var cat in buttons)
                        foreach (var btn in cat)
                ...
                {
                    bool isOwnerPanel = currentCategory == "Owner Panel";
                    bool isSuperAdminPanel = currentCategory == "Admin Test";
                    bool isAdminPanel = currentCategory == "Admin Panel";

                    displayButtons = buttons[localCategory].Where(b =>
                    {
                        // If in owner panel, only owners can see buttons
                        if (isOwnerPanel && !isOwner) return false;

                        // If in super admin panel, only super admins (not owners) can see buttons
                        if (isSuperAdminPanel && (!isSuperAdmin || isOwner)) return false;

                        // If in admin panel, only regular admins (not super admins or owners) can see buttons
                        if (isAdminPanel && (!isAdmin || isSuperAdmin || isOwner)) return false;
